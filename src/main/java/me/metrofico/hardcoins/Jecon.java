...
public class Jecon extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        // コマンドを登録
        getCommand("eco").setExecutor(new Executer(this));
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length > 0 && args[0].equalsIgnoreCase("reload")) {
            ...
            if (!sender.hasPermission("hardcon.reload")) {
                ...
                sender.sendMessage("[" + ChatColor.RED + "Hardcon" + ChatColor.RESET + "] You don't have permission!!");
                return true;
            }
            // リロード
            onEnable();
            // リロード完了のメッセージ([Jecon] Config has been reloaded!!)
            sender.sendMessage("[" + ChatColor.GREEN + "Hardcon" + ChatColor.RESET
                    + "] Config has been reloaded!!");
        } else {
            // エラーメッセージ([Jecon] Initialization Error!!)
            sender.sendMessage("[" + ChatColor.RED + "Hardcon" + ChatColor.RESET + "] Initialization "
                    + ChatColor.RED + "Error" + ChatColor.RESET + "!!");
            // 管理人に連絡して下さいメッセージ([Jecon] Plase contact the administrator!!)
            sender.sendMessage("[" + ChatColor.RED + "Hardcon" + ChatColor.RESET + "] Please " + ChatColor.BOLD
