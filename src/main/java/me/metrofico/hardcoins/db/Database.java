package me.metrofico.hardcoins.db;
...
public abstract class Database {
    public void removeCacheAccount(String name) {
        try {
            balanceCache.remove(getId(name));
        } catch (Exception e) {
    ...
    // キャッシュ
    private Map<UUID, Integer> uuidCache = new HashMap<>();
    private Map<String, Integer> nameCache = new HashMap<>();
    private Map<Integer, Double> balanceCache = new HashMap<>();
    ...
    private int getId(String value, boolean valueIsUuid) {
        ...
        // 接続
        String sql = "SELECT id FROM " + prefix +
                "account WHERE " +
                (valueIsUuid ? "uuid" : "name") +
                "=?";
    ...
    private Map<Integer, String> getName(List<Integer> ids) {
        ...
        String sql = "SELECT name FROM " + prefix +
                "account WHERE id=?";
    ...
    public double getBalanceFromDatabase(String playerName) {
        return getBalanceFromDatabase(getId(playerName));
    }
    ...
    private double getBalance(int id) {
        ...
        // 接続
        String sql = "SELECT balance FROM " + prefix +
                "balance WHERE id=?";
        try (Connection con = this.getConnection();
             PreparedStatement prestat = con.prepareStatement(sql)) {
            // 値をセット
    ...
    public double getBalanceFromDatabase(int id) {
        if (id == -1) {
            return -1;
        }
        // 残高照会(キャッシュから)
    ...
    public void nameUpdate(OfflinePlayer player) {
        ...
        final String oldName = getName(id);
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                // set(name, getBalanceFromDatabase(name));

            }
        }.runTaskAsynchronously(jecon);
    ...
    public String format(double amount) {
        ...
        int tmpMinor = (int) (i * (String.valueOf(i).length() == 3 ? 100 : 100));
        // System.out.println("TmpMinor " + tmpMinor + " I " + i + " length " + String.valueOf(i).length());
        String minor = "";
        if (tmpMinor != 0) {
    ...
    public List<Entry> topList(int page) {
        ...
        // 接続
        String sql = "SELECT id,balance FROM " + prefix +
                "balance ORDER BY balance DESC LIMIT ?,?";
        try (Connection con = this.getConnection();
             PreparedStatement prestat = con.prepareStatement(sql)) {
            // 値をセット
